...
namespace eval RADTOOL {
 ...
 proc RT-master {optin} {
  ...
  set nchangeSrRNA "N/A"
  set nchangeLrRNA "N/A"
  set nalignSrRNA "N/A"
  set nalignLrRNA "N/A"
  ...
  if {[catch {
   lassign [loadrefdata options]  ID_REF_LrRNA ID_REF_SrRNA_1 ID_REF_SrRNA_2   ID_REF_SrRNA_3 ID_REF_SrRNA_4  ID_REF_SrRNA_5 CORE_LrRNA NUM_LrRNA CORE_body NUM_BODY CORE_head NUM_HEAD 
  } errorMessage] != 0} {
  ...
  lassign [defineresidues] body0 body1 body2 head0 head1 head2 body_tilt0_0 body_tilt0_1 head_tilt0_0 head_tilt0_1 body_d head_d absorig
 
  # assign IDs and load structures if necessary
  lassign [loadmodel options] ID_LARGE CHAIN_ID_LrRNA ID_SMALL CHAIN_ID_SrRNA 
  ...
  if { [info exists alignmaps] } {
   lassign [align_renum options $alignmaps $ID_LARGE $CHAIN_ID_LrRNA $ID_SMALL $CHAIN_ID_SrRNA] nalignLrRNA nalignSrRNA nchangeLrRNA nchangeSrRNA
  }
 
  # determine the common atoms between the model and reference
  lassign [docommonstuff options $ID_REF_LrRNA $ID_LARGE $CHAIN_ID_LrRNA $ID_REF_SrRNA_1 $ID_REF_SrRNA_2 $ID_SMALL $CHAIN_ID_SrRNA $CORE_LrRNA $CORE_body $CORE_head] NUM_COMMON_LrRNA COMMON_LrRNA NUM_COMMON_BODY COMMON_BODY NUM_COMMON_HEAD COMMON_HEAD
 
  # prune the cores, if necessary
  if { [info exists options(PRUNE)]} {
   lassign [pruneall options $ID_LARGE $CHAIN_ID_LrRNA $COMMON_LrRNA $ID_REF_LrRNA $ID_SMALL $CHAIN_ID_SrRNA $COMMON_BODY $COMMON_HEAD $ID_REF_SrRNA_1 $ID_REF_SrRNA_2 ] NUM_COMMON_LrRNA COMMON_LrRNA NUM_COMMON_BODY COMMON_BODY NUM_COMMON_HEAD COMMON_HEAD  
  }
...
Generated by RADtool v$radversion, on [clock format [clock second] -format "%D %T"], using machine [info hostname]"
   if { ! [info exists options(ONLYSSU)] } {
    puts $options(CORESOUT) "LSUCORE $COMMON_LrRNA" 
   }
   if { ! [info exists options(ONLYLSU)] } {
    puts $options(CORESOUT) "BODYCORE $COMMON_BODY" 
    puts $options(CORESOUT) "HEADCORE $COMMON_HEAD" 
   }
  ...
  if { [info exists options(LOADTRAJ)]} { 
   if { ! [info exists options(ONLYSSU)] } {
    puts $options(OUTPUT) "# sequence matching: $NUM_COMMON_LrRNA large rRNA residues, $NUM_COMMON_BODY small rRNA body residues, $NUM_COMMON_HEAD small rRNA head residues"
   } else {
    puts $options(OUTPUT) "# sequence matching: $NUM_COMMON_BODY small rRNA body residues, $NUM_COMMON_HEAD small rRNA head residues"
   }
  }
  if { ! [info exists options(ONLYSSU)] && $NUM_COMMON_LrRNA==0 } {
   puts $options(ERROR) "\nError: Must have at least one common residue in the large subunit. Found: $NUM_COMMON_LrRNA \n"
   closelogs options
   return
  }
  if { ! [info exists options(ONLYLSU)] && ($NUM_COMMON_BODY ==0 || $NUM_COMMON_HEAD==0 ) } {
   puts $options(ERROR) "\nError: Must have at least one common residue in the small head and small body. Found: $NUM_COMMON_HEAD, $NUM_COMMON_BODY \n";
   closelogs options
   return
  }
  
  # set the representations for all of the rigid bodies
  drawrefs options $ID_REF_LrRNA $ID_REF_SrRNA_1 $ID_REF_SrRNA_2 $ID_REF_SrRNA_3 $ID_REF_SrRNA_4 $ID_REF_SrRNA_5 $COMMON_LrRNA $COMMON_BODY $COMMON_HEAD 
  ##############DEFINE ATOM SELECTIONS ##################
  if { ! [info exists options(ONLYSSU)] } {
   set p0_LrRNA [atomselect $ID_REF_LrRNA "resid $COMMON_LrRNA and name P and not insertion A to Z"]
   set p1_LrRNA [atomselect $ID_LARGE "chain \"$CHAIN_ID_LrRNA\" and resid $COMMON_LrRNA and name P and not insertion A to Z and not altloc B to Z"]
   set large [atomselect $ID_LARGE all]
   if { ! [info exists options(ONLYLSU)] } {
    set body_c_0 [get_xyz $ID_REF_SrRNA_1 $body1]  
   } else {
    set body_c_0 0
   }
  } else {
   set p0_LrRNA 0
   set p1_LrRNA 0
   # we only need this defined, but we shouldn't use it.
   set bodytvecs { {1 0 0} {0 1 0} {0 0 1}}
   set large 0
   set body_c_0 0
  }
  
  if { ! [info exists options(ONLYLSU)] } {
   set p1_body [atomselect $ID_SMALL "chain \"$CHAIN_ID_SrRNA\" and resid $COMMON_BODY and name P and not insertion A to Z and not altloc B to Z"]
   set p0_body [atomselect $ID_REF_SrRNA_1 "resid $COMMON_BODY and name P and not insertion A to Z"]
   set p1_head [atomselect $ID_SMALL "chain \"$CHAIN_ID_SrRNA\" and resid $COMMON_HEAD and name P and not insertion A to Z and not altloc B to Z"]
   set p0_head [atomselect $ID_REF_SrRNA_2 "resid $COMMON_HEAD and name P and not insertion A to Z"]
   set ref16_1 [atomselect $ID_REF_SrRNA_1 "all"]
   set ref16_2 [atomselect $ID_REF_SrRNA_2 "all"]
   set small [atomselect $ID_SMALL "all"]
   ###########END ATOM SELECTIONS#########################
   # some body reference values
   lassign [get_vectors $ID_REF_SrRNA_1 $body0 $body1 $body2]  ref_bond_body ref_norv_body ref_perp_body
   set zero_body [ get_one_vector $ID_REF_SrRNA_1 $body_tilt0_0 $body_tilt0_1 ]
  } else {
   set p0_body 0
   set p1_body 0
   set p0_head 0
   ...
   set small 0
   # some body reference values
   set ref_bond_body {0 0 0} 
   set ref_norv_body {0 0 0}
   set ref_perp_body {0 0 0}
   set zero_body {0 0 0}
  }
  ...
  if { [info exists options(ONLYLSU)] } {
   ...
   set LSURMSD [do_all_angles options $ref16_1 $ref16_2 $p0_LrRNA $p1_LrRNA $p1_body $p0_body $p1_head $p0_head $zero_body $large $small 0 $ID_LARGE $ID_SMALL $CHAIN_ID_LrRNA $CHAIN_ID_SrRNA $NUM_LrRNA $NUM_BODY $NUM_HEAD  $NUM_COMMON_LrRNA $NUM_COMMON_BODY $NUM_COMMON_HEAD  $ID_REF_SrRNA_1 $ID_REF_SrRNA_2  $ID_REF_SrRNA_3 $ID_REF_SrRNA_4 $ID_REF_SrRNA_5 $ref_norv_body $ref_perp_body $ref_bond_body $body0 $body1 $body2 $body_d $body_c_0 $head0 $head1 $head2 $head_d $head_tilt0_0 $head_tilt0_1 $nchangeSrRNA $nchangeLrRNA $nalignSrRNA $nalignLrRNA $absorig]
   set LSURMSD [format %.2f $LSURMSD]
   puts $options(OUTPUT) "RMSD of LSU core between ref. and model: $LSURMSD" 
  } else {
 
   if {! [info exists options(isTraj)] } {
    set framen 0
    # if not a trajectory, just analyze the loaded frame
    set out [do_all_angles options $ref16_1 $ref16_2 $p0_LrRNA $p1_LrRNA $p1_body $p0_body $p1_head $p0_head $zero_body $large $small $framen $ID_LARGE $ID_SMALL $CHAIN_ID_LrRNA $CHAIN_ID_SrRNA $NUM_LrRNA $NUM_BODY $NUM_HEAD  $NUM_COMMON_LrRNA $NUM_COMMON_BODY $NUM_COMMON_HEAD  $ID_REF_SrRNA_1 $ID_REF_SrRNA_2  $ID_REF_SrRNA_3 $ID_REF_SrRNA_4 $ID_REF_SrRNA_5 $ref_norv_body $ref_perp_body $ref_bond_body $body0 $body1 $body2 $body_d $body_c_0 $head0 $head1 $head2 $head_d $head_tilt0_0 $head_tilt0_1 $nchangeSrRNA $nchangeLrRNA $nalignSrRNA $nalignLrRNA  $absorig]
 
    findbothtrios $ID_REF_SrRNA_1 $ID_REF_SrRNA_2 $ID_REF_SrRNA_3 $ID_REF_SrRNA_4 $COMMON_BODY $COMMON_HEAD
    ...
    animate delete  beg 0 end -1 skip 0 $ID_LARGE
    ...
    for {set i 1} {$i < [llength $options(LOADTRAJ)]} {incr i} {
     ...
     while {$seglength == $nf} {
      ...
      animate delete  beg 0 end -1 skip 0 $ID_LARGE
      # load on traj file
      mol addfile $tfile first $loadnumb last $loadnume step $options(TRAJSTEP) waitfor -1 $ID_LARGE
 
      set nf [molinfo $ID_LARGE get numframes]
      # LOOP OVER TRAJECTORY
      for {set framen 0} {$framen < $nf} {incr framen} { 
       # update the frame
       $p1_LrRNA frame $framen
       $p1_body frame $framen
       $p1_head frame $framen
 
       do_all_angles options $ref16_1 $ref16_2 $p0_LrRNA $p1_LrRNA $p1_body $p0_body $p1_head $p0_head $zero_body $large $small $framen $ID_LARGE $ID_SMALL $CHAIN_ID_LrRNA $CHAIN_ID_SrRNA $NUM_LrRNA $NUM_BODY $NUM_HEAD  $NUM_COMMON_LrRNA $NUM_COMMON_BODY $NUM_COMMON_HEAD  $ID_REF_SrRNA_1 $ID_REF_SrRNA_2 $ID_REF_SrRNA_3 $ID_REF_SrRNA_4 $ID_REF_SrRNA_5 $ref_norv_body $ref_perp_body $ref_bond_body $body0 $body1 $body2 $body_d $body_c_0 $head0 $head1 $head2 $head_d $head_tilt0_0 $head_tilt0_1 $nchangeSrRNA $nchangeLrRNA $nalignSrRNA $nalignLrRNA $absorig 
    ...
    for {set i 1} {$i < [llength $options(LOADTRAJ)]} {incr i} {
     ...
     animate delete  beg 0 end -1 skip 0 $ID_LARGE
     # load on traj file
     mol addfile $tfile first $options(TRAJFIRST) last $options(TRAJLAST) step $options(TRAJSTEP) waitfor -1 $ID_LARGE
     set nf [molinfo $ID_LARGE get numframes]
     # LOOP OVER TRAJECTORY
     for {set framen 0} {$framen < $nf} {incr framen} { 
      # update the frame
  ...
  if {! [info exists options(LOADTRAJ)]} {
   finalwrapup options $ID_LARGE $ID_SMALL $CHAIN_ID_LrRNA $CHAIN_ID_SrRNA $ID_REF_SrRNA_5
  }
  closelogs options
...
"
 ...
 proc loadrefdata {optin} {
  ...
  return [list $ID_REF_LrRNA $ID_REF_SrRNA_1 $ID_REF_SrRNA_2  $ID_REF_SrRNA_3  $ID_REF_SrRNA_4 $ID_REF_SrRNA_5 $CORE_LrRNA $NUM_LrRNA $CORE_body $NUM_BODY $CORE_head $NUM_HEAD]
 }
 
 proc do_all_angles {optin ref16_1 ref16_2 p0_LrRNA p1_LrRNA p1_body p0_body p1_head p0_head zero_body large small framen ID_LARGE ID_SMALL CHAIN_ID_LrRNA CHAIN_ID_SrRNA NUM_LrRNA NUM_BODY NUM_HEAD NUM_COMMON_LrRNA NUM_COMMON_BODY NUM_COMMON_HEAD ID_REF_SrRNA_1 ID_REF_SrRNA_2 ID_REF_SrRNA_3 ID_REF_SrRNA_4  ID_REF_SrRNA_5 ref_norv_body ref_perp_body ref_bond_body body0 body1 body2 body_d body_c_0 head0 head1 head2 head_d head_tilt0_0 head_tilt0_1 nchangeSrRNA nchangeLrRNA nalignSrRNA nalignLrRNA absorig} {
 ...
 proc align_renum {optin alignmaps ID_LARGE CHAIN_ID_LrRNA ID_SMALL CHAIN_ID_SrRNA} {
  # this either applies a sequence alignment, STAMP alignment, or reads in the alignments and renumbers residues.
  upvar $optin options
  set nalignLrRNA 0
  set nchangeLrRNA 0
  ...
  return [list $nalignLrRNA $nalignSrRNA $nchangeLrRNA $nchangeSrRNA]
 }
 
 proc docommonstuff {optin ID_REF_LrRNA ID_LARGE CHAIN_ID_LrRNA ID_REF_SrRNA_1 ID_REF_SrRNA_2 ID_SMALL CHAIN_ID_SrRNA CORE_LrRNA CORE_body CORE_head } {
  ...
  return [list $NUM_COMMON_LrRNA $COMMON_LrRNA $NUM_COMMON_BODY $COMMON_BODY $NUM_COMMON_HEAD $COMMON_HEAD]
 }
 
 proc pruneall {optin ID_LARGE CHAIN_ID_LrRNA COMMON_LrRNA ID_REF_LrRNA ID_SMALL CHAIN_ID_SrRNA COMMON_BODY COMMON_HEAD ID_REF_SrRNA_1 ID_REF_SrRNA_2} {
 ...
 proc drawrefs {optin ID_REF_LrRNA ID_REF_SrRNA_1 ID_REF_SrRNA_2 ID_REF_SrRNA_3 ID_REF_SrRNA_4 ID_REF_SrRNA_5 COMMON_LrRNA COMMON_BODY COMMON_HEAD} {
 ...
 proc findbothtrios {ID_REF_SrRNA_1 ID_REF_SrRNA_2 ID_REF_SrRNA_3 ID_REF_SrRNA_4 COMMON_BODY COMMON_HEAD} {
 ...
 # which residues are used to define the zero direction for the tilting axis
  ...
  return [list $body0 $body1 $body2 $head0 $head1 $head2 $body_tilt0_0 $body_tilt0_1 $head_tilt0_0 $head_tilt0_1 $body_d $head_d $absorig]
 }
 ...
 proc loadmodel {optin} {
  ...
  return [list $ID_LARGE $CHAIN_ID_LrRNA $ID_SMALL $CHAIN_ID_SrRNA ]
 }
...
Generated by RADtool v$radversion, on [clock format [clock second] -format "%D %T"], using machine [info hostname]"
 ...
 proc findseqaligned { optin seqlistonly} {
  ...
  if { ! [info exists options(ONLYLSU)] } {
   ...
   set nalignSrRNA 0
   #make a mapping array from original numbers to E coli-aligned numbering
   for {set i 0} { $i <[::SeqData::getSeqLength $seqIDSrRNAalign]} {incr i} {
...
"
 ...
 proc finalwrapup {optin ID_LARGE ID_SMALL CHAIN_ID_LrRNA CHAIN_ID_SrRNA ID_REF_SrRNA_5} {
  variable radenv
  upvar $optin options
