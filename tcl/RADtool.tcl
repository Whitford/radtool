...
namespace eval RADTOOL {
 ...
 proc RADtool args {
  # This is the proc used to call RADtool from the command line 
  variable pi 
  variable radenv
 ...
 proc RT-master {optin} {
  # this it the proc that is used internally to control the workflow
  upvar $optin options
  variable radenv
  variable radversion
  variable RTD
  ...
  # perform an initial sequence/structure alignment
  ...
  # set the residues that define rigid-body axes
  defineresidues
  ...
  # print information about common residues
  printcommoninfo options
 
  # set the representations for all of the rigid bodies
  drawrefs options

  # create atom selections for the common residues
  selectcommon options
 
  if {! [info exists options(isTraj)] } {
   # not a trajectory, just analyze the loaded frame
...
"
    ...
    if { [lindex $list 0] eq "large" &&  [lindex $list 1] eq "subunit:" } {
     ...
     if { [info exists options(ONLYSSU)] } {
      ...
      if { [expr [llength $list] % 2] != 0 } {
       error "Alignment file has an odd number of residue numbers listed
 for the LSU. The file should list pairs of numbers (model and E. coli number)"
      }
 ...
 proc align_renum {optin } {
  # This renumbers based on the earlier alignment
  upvar $optin options
  variable RTD
...
Generated by RADtool v$radversion, on [clock format [clock second] -format "%D %T"], using machine [info hostname]"
 ...
 proc selectcommon {optin} {
  # select the cores and generate some reference vectors
  upvar $optin options
  variable RTD

  if { ! [info exists options(ONLYSSU)] } {
   set RTD(p0_LrRNA) [uplevel "#0" [list atomselect $RTD(ID_REF_LrRNA) "resid $RTD(COMMON_LrRNA) and name P and not insertion A to Z"]]
   set RTD(p1_LrRNA) [uplevel "#0"  [list atomselect $RTD(ID_LARGE) "chain \"$RTD(CHAIN_ID_LrRNA)\" and resid $RTD(COMMON_LrRNA) and name P and not insertion A to Z and not altloc B to Z"]]
   set RTD(large) [uplevel "#0"  [list atomselect $RTD(ID_LARGE) "all"]]
   if { ! [info exists options(ONLYLSU)] } {
  ...
  if { ! [info exists options(ONLYLSU)] } {
   set RTD(p1_body) [uplevel "#0"  [list atomselect $RTD(ID_SMALL) "chain \"$RTD(CHAIN_ID_SrRNA)\" and resid $RTD(COMMON_BODY) and name P and not insertion A to Z and not altloc B to Z"]]
   set RTD(p0_body) [uplevel "#0"  [list atomselect $RTD(ID_REF_SrRNA_1) "resid $RTD(COMMON_BODY) and name P and not insertion A to Z"]]
   set RTD(p1_head) [uplevel "#0"  [list atomselect $RTD(ID_SMALL) "chain \"$RTD(CHAIN_ID_SrRNA)\" and resid $RTD(COMMON_HEAD) and name P and not insertion A to Z and not altloc B to Z"]]
   set RTD(p0_head) [uplevel "#0"  [list atomselect $RTD(ID_REF_SrRNA_2) "resid $RTD(COMMON_HEAD) and name P and not insertion A to Z"]]
   set RTD(ref16_1) [uplevel "#0"  [list atomselect $RTD(ID_REF_SrRNA_1) "all"]]
   set RTD(ref16_2) [uplevel "#0"  [list atomselect $RTD(ID_REF_SrRNA_2) "all"]]
   set RTD(small) [uplevel "#0"  [list atomselect $RTD(ID_SMALL) "all"]]
 ...
 proc cleanandclose {optin} {
  ...
  if { ! [info exists options(ONLYSSU)] } {
   uplevel "#0" $RTD(p0_LrRNA) delete 
   uplevel "#0" $RTD(p1_LrRNA)  delete
   uplevel "#0" $RTD(large)  delete
  }
  
  if { ! [info exists options(ONLYLSU)] } {
   uplevel "#0" $RTD(p1_body)  delete
   uplevel "#0" $RTD(p0_body)  delete
   uplevel "#0" $RTD(p1_head)  delete
   uplevel "#0" $RTD(p0_head)  delete
   uplevel "#0" $RTD(ref16_1)  delete
   uplevel "#0" $RTD(ref16_2)  delete
   uplevel "#0" $RTD(small)  delete
  }
 }
 ...
 proc doseqstampalign { optin } {
  ...
  if { ! [info exists options(ONLYLSU)] } {
   ...
   if {$seqlistonly == 1} {
    ...
    if {[info exists options(FINDHEAD3)]} { 
     set tentativehead1 [atomselect $ID_SMALL "not backbone and not name P and chain \"$CHAIN_ID_SrRNA\" and $resrange1 and name \"\[a-zA-Z0-9\]*\" and noh"]
     set tentativehead2 [atomselect $ID_SMALL "not backbone and not name P and chain \"$CHAIN_ID_SrRNA\" and $resrange2 and name \"\[a-zA-Z0-9\]*\" and noh"]
    } else {
    ...
    if { $nconts == 0 } {
     error "sequence-based alignment could not make a good guess about where the SSU head is located. This error is common when trying to analyze atypical ribosomes (e.g. those that are composed of many rRNA molecules per subunit). If this is a structure composed of large rRNA molecules in the LSU and SSU (e.g. 23S and 16S), then perhaps try using the -h_f and -h_l options. It can also happen if there are issues in the head residue numbering. The flag -findhead2 (Alt. find head method button) or -findhead3 may be helpful." 
    }
...
Generated by RADtool v$radversion, on [clock format [clock second] -format "%D %T"], using machine [info hostname]"
 ...
 proc dostructurealignments {optin PDBNAME CHAIN_ID map PDBNAME_REF CHAIN_ID_REF lhb head_b head_e} {
  ...
  if { $lhb eq "head" } {
   ...
   set selstring "chain \"$CHAIN_ID\"  and (resid 0 to 1000000) and (not resid $head_b to $head_e)"
  } elseif { $lhb eq "all" } { 
   set selstring "chain \"$CHAIN_ID\" and (resid 0 to 1000000)" 
  } else {
 ...
 proc prunecore {optin MOL_ID CHAIN_ID COMMON ID_REF} {
  # Remove anything over a threshold and add anything under.  Then, realign based on those under and add/remove.  Continue until convergence is reached.
  ...
  while { $diff > 0 } { 
   ...
   if {[llength $newlist] == 0} {
    error "During pruning, all residues were discarded. This generally happens for \"unusual\" ribosomes. However, sometimes this just means you need to assign E coli numbering via the -stamp flag.  This specific issue occurred when trying to align molecules $ID_REF and $newID."
   }
   if { $oldlist == $newlist } {
    set diff 0
   } else {
...
When using the command-line interface, the following flags are supported:
   ...
   Alignment options:
       ...
       -findhead3        : Another alternate method for guessing the head location
